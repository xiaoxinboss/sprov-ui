...
install() {
    install_base
    bash <(curl -L -s https://github.com/xiaoxinboss/sprov-ui/blob/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://github.com/xiaoxinboss/sprov-ui/blob/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/xiaoxinboss/sprov-ui/blob/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
